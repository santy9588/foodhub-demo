    ...
    <div className="container py-12 max-w-2xl">
      <div className="text-center mb-8 space-y-4">
        ...
        <p className="text-muted-foreground">
          Your order has been placed successfully. Order ID: <span className="font-mono">{order.id}</span>
        </p>
      </div>
            ...
            <div className="space-y-2">
              {order.items.map((item) => (
                <div key={item.item.id} className="flex justify-between text-sm">
                  ...
                  <span>₹{((Number(item.item.price) * Number(item.quantity)) / 100).toFixed(2)}</span>
                </div>
              ))}
            </div>
          </div>
